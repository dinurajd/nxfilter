...
function set_period(form){
	...
	var stime = '<%= strftime_add("yyyy/MM/dd", -86400 * 7)%>';

	if(opt == '1week'){
		stime = '<%= strftime_add("yyyy/MM/dd", -86400 * 7 * 2)%>';
	}

	if(opt == '2weeks'){
		stime = '<%= strftime_add("yyyy/MM/dd", -86400 * 7 * 3)%>';
	}

	if(opt == '3weeks'){
		stime = '<%= strftime_add("yyyy/MM/dd", -86400 * 7 * 4)%>';
	}
...
google.setOnLoadCallback(draw_clt_ip_top);
function draw_clt_ip_top() {
	var data = google.visualization.arrayToDataTable([
		["IP", "Request"]
<%
...
<table width="100%" cellpadding="4" class="view">
	...
	Report from :
		...
		<td>
			...
			<input type="radio" class="no_border" name="time_option" value="last7days" onclick="javascript:set_period(this.form)"
				<%if(g_time_option.equals("last7days")){out.print("checked");}%>> Last 7 days
			<input type="radio" class="no_border" name="time_option" value="1week" onclick="javascript:set_period(this.form)"
				<%if(g_time_option.equals("1week")){out.print("checked");}%>> 1 week ago
			<input type="radio" class="no_border" name="time_option" value="2weeks" onclick="javascript:set_period(this.form)"
				<%if(g_time_option.equals("2weeks")){out.print("checked");}%>> 2 weeks ago
			<input type="radio" class="no_border" name="time_option" value="3weeks" onclick="javascript:set_period(this.form)"
				<%if(g_time_option.equals("3weeks")){out.print("checked");}%>> 3 weeks ago
		</td>
	</tr>
